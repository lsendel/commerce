  ...
  return (
    <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        {collections.length === 0 ? (
          ...
          <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
            <table class="w-full text-sm">
              ...
              <tbody id="collections-tbody" class="divide-y divide-gray-100 dark:divide-gray-700">
                {collections.map((col) => (
                  <tr
                    ...
                    data-product-count={String(col.productCount ?? 0)}
                  >
                    <td class="px-4 py-3">
                      <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-lg bg-gray-100 overflow-hidden flex-shrink-0" data-collection-thumb>
                          {col.imageUrl ? (
                        ...
                        <span class="font-medium text-gray-900 dark:text-gray-100" data-collection-name>{col.name}</span>
                      </div>
                    </td>
                    <td class="px-4 py-3 text-gray-500" data-collection-slug>/{col.slug}</td>
                    <td class="px-4 py-3 text-gray-600" data-collection-products>{col.productCount}</td>
      ...
      {html`
        <script>
          (function() {
            var listRoot = document.getElementById('collection-list-root');
            ...
            function normalizeCollection(raw) {
              ...
              return {
                id: id,
                name: raw.name ? String(raw.name) : 'Untitled collection',
            ...
            function renderThumb(collection) {
              if (collection.imageUrl) {
                return '<img src="' + escapeHtml(collection.imageUrl) + '" alt="' + escapeHtml(collection.name) + '" class="w-full h-full object-cover" />';
              }
              return '' +
                '<div class="w-full h-full flex items-center justify-center text-gray-300">' +
                  '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">' +
            ...
            function renderCollectionRow(collection) {
              return '' +
                '<tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors" data-collection-row data-id="' + escapeHtml(collection.id) + '" data-name="' + escapeHtml(collection.name) + '" data-slug="' + escapeHtml(collection.slug) + '" data-description="' + escapeHtml(collection.description) + '" data-image-url="' + escapeHtml(collection.imageUrl) + '" data-product-count="' + collection.productCount + '">' +
                  '<td class="px-4 py-3">' +
                    '<div class="flex items-center gap-3">' +
                      '<div class="w-10 h-10 rounded-lg bg-gray-100 overflow-hidden flex-shrink-0" data-collection-thumb>' + renderThumb(collection) + '</div>' +
                      '<span class="font-medium text-gray-900 dark:text-gray-100" data-collection-name>' + escapeHtml(collection.name) + '</span>' +
                    '</div>' +
                  '</td>' +
                  '<td class="px-4 py-3 text-gray-500" data-collection-slug>/' + escapeHtml(collection.slug) + '</td>' +
                  '<td class="px-4 py-3 text-gray-600" data-collection-products>' + collection.productCount + '</td>' +
                  '<td class="px-4 py-3 text-right">' +
                    '<div class="flex items-center justify-end gap-1">' +
                      '<button type="button" class="btn-edit-collection p-1.5 rounded-lg text-gray-400 hover:text-brand-600 hover:bg-brand-50 transition-colors" title="Edit">' +
                        '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">' +
                          '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>' +
                        '</svg>' +
                      '</button>' +
                      '<button type="button" class="btn-delete-collection p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Delete">' +
                        '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">' +
              ...
              if (!Array.isArray(collections) || collections.length === 0) {
                listRoot.innerHTML = '' +
                  '<div class="flex flex-col items-center justify-center py-12 px-6 text-center">' +
                    '<h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">No collections yet</h3>' +
                    '<p class="mt-1 text-sm text-gray-500 dark:text-gray-400 max-w-sm">Create your first collection to organize products.</p>' +
                  '</div>';
                return;
              }
              listRoot.innerHTML = '' +
                '<div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">' +
                  '<table class="w-full text-sm">' +
                    ...
                    '</thead>' +
                    '<tbody id="collections-tbody" class="divide-y divide-gray-100 dark:divide-gray-700">' +
                      collections.map(renderCollectionRow).join('') +
                    '</tbody>' +
                  '</table>' +
                '</div>';
            }
            ...
            async function refreshCollections() {
              var res = await fetch('/api/admin/collections');
              var data = await res.json().catch(function() { return {}; });
              if (!res.ok) {
                throw new Error(window.petm8GetApiErrorMessage ? window.petm8GetApiErrorMessage(data, 'Failed to refresh collections') : (data.error || data.message || 'Failed to refresh collections'));
              }
              var collections = Array.isArray(data) ? data.map(normalizeCollection).filter(Boolean) : [];
            ...
            document.addEventListener('click', async function(event) {
              var editBtn = event.target && event.target.closest ? event.target.closest('.btn-edit-collection') : null;
              if (editBtn) {
                var editRow = editBtn.closest('[data-collection-row]');
                populateFormFromRow(editRow);
                showForm('Edit Collection');
                return;
              }

              var deleteBtn = event.target && event.target.closest ? event.target.closest('.btn-delete-collection') : null;
              if (!deleteBtn) return;

              var row = deleteBtn.closest('[data-collection-row]');
              if (!row || !row.dataset.id) return;
              var approved = await confirmDialog('Delete collection "' + (row.dataset.name || 'this collection') + '"?');
              if (!approved) return;

              deleteBtn.setAttribute('disabled', 'true');
              try {
                var res = await fetch('/api/admin/collections/' + row.dataset.id, { method: 'DELETE' });
                var data = await res.json().catch(function() { return {}; });
                if (!res.ok) {
                  throw new Error(window.petm8GetApiErrorMessage ? window.petm8GetApiErrorMessage(data, 'Delete failed') : (data.error || data.message || 'Delete failed'));
                }
                await refreshCollections();
                flash(successEl, 'Collection deleted.');
              } catch (err) {
                flash(errorEl, (err && err.message) || 'Failed to delete collection.');
              } finally {
            ...
            form.addEventListener('submit', async function(e) {
              ...
              try {
                ...
                var data = await res.json().catch(function() { return {}; });
                if (!res.ok) {
                  throw new Error(window.petm8GetApiErrorMessage ? window.petm8GetApiErrorMessage(data, 'Save failed') : (data.error || data.message || 'Save failed'));
                }
                await refreshCollections();
                hideForm();
                flash(successEl, id ? 'Collection updated.' : 'Collection created.');
              } catch (err) {
                flash(errorEl, (err && err.message) || 'Failed to save collection.');
              } finally {
