  ...
  return (
    <div class="max-w-6xl mx-auto px-4 py-8">
      <div class="flex items-center justify-between mb-8">
        <div>
          <nav class="text-xs text-gray-400 mb-1">
            ...
            <span class="text-gray-600">Tax Settings</span>
          </nav>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Tax Settings</h1>
        </div>
        <button
          type="button"
          id="btn-add-zone"
      ...
      <div id="zone-form-section" class="hidden mb-6 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6">
        <h2 id="zone-form-title" class="text-lg font-semibold text-gray-900 mb-4">Add Tax Zone</h2>
        <form id="zone-form" onsubmit="return false;" class="space-y-4">
          <input type="hidden" name="zoneId" value="" />
          <Input label="Zone Name" name="zoneName" required placeholder="e.g. US Sales Tax" />
          <div>
      ...
      {zones.length === 0 ? (
        <div class="text-center py-12">
          ...
          <h3 class="text-sm font-medium text-gray-900">No tax zones configured</h3>
          <p class="mt-1 text-sm text-gray-500">Create your first tax zone to start collecting taxes on orders.</p>
        </div>
      ) : (
        <div class="space-y-6">
          {zones.map((zone) => (
            <div key={zone.id} class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden" data-zone-id={zone.id}>
                ...
                <div>
                  ...
                  <div class="flex flex-wrap gap-1 mt-1">
                    {zone.countries.slice(0, 5).map((c) => (
                      <span class="inline-flex items-center px-2 py-0.5 rounded text-xs bg-gray-100 text-gray-700">{c}</span>
                    ))}
              ...
              <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900">
                  <tr>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Rate Name</th>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Rate %</th>
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Compound</th>
                  ...
                  {zone.rates.length === 0 ? (
                    ...
                    zone.rates.map((rate) => (
                      <tr key={rate.id}>
                        ...
                        <td class="px-4 py-2 text-sm font-medium">{rate.rate}%</td>
                        <td class="px-4 py-2 text-sm">{rate.isCompound ? "Yes" : "No"}</td>
                        <td class="px-4 py-2 text-sm text-gray-500">{rate.priority}</td>
                      </tr>
                    ))
                  )}
                </tbody>
        ...
        <script>
          (function() {
            ...
            form.addEventListener('submit', async function(e) {
              ...
              var url = zoneId ? '/api/admin/tax/zones/' + zoneId : '/api/admin/tax/zones';
            ...
            document.querySelectorAll('[data-delete-zone]').forEach(function(btn) {
              btn.addEventListener('click', async function() {
                if (!confirm('Delete this tax zone?')) return;
                var id = this.getAttribute('data-delete-zone');
                try {
                  var res = await fetch('/api/admin/tax/zones/' + id, { method: 'DELETE' });
