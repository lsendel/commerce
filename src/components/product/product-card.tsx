...
const typeBadgeConfig: Record<string, { label: string; classes: string }> = {
  digital: { label: "Digital", classes: "bg-purple-100 text-purple-700" },
  subscription: { label: "Subscription", classes: "bg-blue-100 text-blue-700" },
  bookable: { label: "Event", classes: "bg-pet-teal/20 text-teal-700" },
...
export const ProductCard: FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <a
      href={`/products/${slug}`}
      class="group block rounded-2xl bg-white shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden border border-gray-100"
    >
      {/* Image */}
      <div class="relative aspect-square overflow-hidden bg-gray-50">
        ...
        {/* Sale badge */}
        {isOnSale && (
          <span class="absolute top-3 right-3 text-xs font-semibold px-2.5 py-1 rounded-full bg-red-100 text-red-700">
            Sale
          </span>
        )}
      </div>
      ...
      {/* Info */}
      <div class="p-4">
        <h3 class="text-sm font-semibold text-gray-900 group-hover:text-brand-600 transition-colors duration-200 line-clamp-2 mb-2">
          {name}
        </h3>
        <div class="flex items-center gap-2 flex-wrap">
          {isMultiVariant ? (
            ...
            <span class={`text-sm font-bold ${isOnSale ? "text-red-600" : "text-gray-900"}`}>
              ${minPrice.toFixed(2)}
            </span>
          )}
